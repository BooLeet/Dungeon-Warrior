...
public class Door : Interactable {
    ...
    {
        if (doorState == DoorState.Open)
            return GameManager.instance.languagePack.doorClose;
        else
        {
            if (doorState == DoorState.Locked)
                return GameManager.instance.languagePack.doorLocked;
            else if (doorState == DoorState.KeyRequired)
            {
                if(interactingCharacter.inventory.HasLoot(keyLoot))
                    return GameManager.instance.languagePack.doorUnlock;
                else
                    return GameManager.instance.languagePack.doorKeyRequired;
            }
                
            return GameManager.instance.languagePack.doorOpen;
        }
    }
}
